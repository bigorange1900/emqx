...
encode_msg(Msg, MsgName, Opts) ->
    ...
    case MsgName of
	  ...
	  encode_msg_initial_request(id(Msg, TrUserData),
				     TrUserData);
      init_response ->
	  encode_msg_init_response(id(Msg, TrUserData),
				   TrUserData);
...
encode_msg_init_response(Msg, TrUserData) ->
    encode_msg_init_response(Msg, <<>>, TrUserData).


encode_msg_init_response(#{} = M, Bin, TrUserData) ->
    case M of
      #{hooks := F1} ->
	  TrF1 = id(F1, TrUserData),
	  if TrF1 == [] -> Bin;
	     true ->
		 e_field_init_response_hooks(TrF1, Bin, TrUserData)
	  end;
      _ -> Bin
    end.
...
e_field_init_response_hooks([Elem | Rest], Bin,
			    TrUserData) ->
    Bin2 = <<Bin/binary, 10>>,
    Bin3 = e_type_string(id(Elem, TrUserData), Bin2,
			 TrUserData),
    e_field_init_response_hooks(Rest, Bin3, TrUserData);
e_field_init_response_hooks([], Bin, _TrUserData) ->
    Bin.
...
decode_msg_2_doit(initial_request, Bin, TrUserData) ->
    id(decode_msg_initial_request(Bin, TrUserData),
       TrUserData);
decode_msg_2_doit(init_response, Bin, TrUserData) ->
    id(decode_msg_init_response(Bin, TrUserData),
       TrUserData);
...
decode_msg_init_response(Bin, TrUserData) ->
    dfp_read_field_def_init_response(Bin, 0, 0,
				     id([], TrUserData), TrUserData).

dfp_read_field_def_init_response(<<10, Rest/binary>>,
				 Z1, Z2, F@_1, TrUserData) ->
    d_field_init_response_hooks(Rest, Z1, Z2, F@_1,
				TrUserData);
dfp_read_field_def_init_response(<<>>, 0, 0, R1,
				 TrUserData) ->
    #{hooks => lists_reverse(R1, TrUserData)};
dfp_read_field_def_init_response(Other, Z1, Z2, F@_1,
				 TrUserData) ->
    dg_read_field_def_init_response(Other, Z1, Z2, F@_1,
				    TrUserData).

dg_read_field_def_init_response(<<1:1, X:7,
				  Rest/binary>>,
				N, Acc, F@_1, TrUserData)
    when N < 32 - 7 ->
    dg_read_field_def_init_response(Rest, N + 7,
				    X bsl N + Acc, F@_1, TrUserData);
dg_read_field_def_init_response(<<0:1, X:7,
				  Rest/binary>>,
				N, Acc, F@_1, TrUserData) ->
    Key = X bsl N + Acc,
    case Key of
      10 ->
	  d_field_init_response_hooks(Rest, 0, 0, F@_1,
				      TrUserData);
      _ ->
	  case Key band 7 of
	    0 ->
		skip_varint_init_response(Rest, 0, 0, F@_1, TrUserData);
	    1 ->
		skip_64_init_response(Rest, 0, 0, F@_1, TrUserData);
	    2 ->
		skip_length_delimited_init_response(Rest, 0, 0, F@_1,
						    TrUserData);
	    3 ->
		skip_group_init_response(Rest, Key bsr 3, 0, F@_1,
					 TrUserData);
	    5 -> skip_32_init_response(Rest, 0, 0, F@_1, TrUserData)
	  end
    end;
dg_read_field_def_init_response(<<>>, 0, 0, R1,
				TrUserData) ->
    #{hooks => lists_reverse(R1, TrUserData)}.

d_field_init_response_hooks(<<1:1, X:7, Rest/binary>>,
			    N, Acc, F@_1, TrUserData)
    when N < 57 ->
    d_field_init_response_hooks(Rest, N + 7, X bsl N + Acc,
				F@_1, TrUserData);
d_field_init_response_hooks(<<0:1, X:7, Rest/binary>>,
    ...
    dfp_read_field_def_init_response(RestF, 0, 0,
				     cons(NewFValue, Prev, TrUserData),
				     TrUserData).

skip_varint_init_response(<<1:1, _:7, Rest/binary>>, Z1,
			  Z2, F@_1, TrUserData) ->
    skip_varint_init_response(Rest, Z1, Z2, F@_1,
			      TrUserData);
skip_varint_init_response(<<0:1, _:7, Rest/binary>>, Z1,
			  Z2, F@_1, TrUserData) ->
    dfp_read_field_def_init_response(Rest, Z1, Z2, F@_1,
				     TrUserData).

skip_length_delimited_init_response(<<1:1, X:7,
				      Rest/binary>>,
				    N, Acc, F@_1, TrUserData)
    when N < 57 ->
    skip_length_delimited_init_response(Rest, N + 7,
					X bsl N + Acc, F@_1, TrUserData);
skip_length_delimited_init_response(<<0:1, X:7,
				      Rest/binary>>,
				    N, Acc, F@_1, TrUserData) ->
    Length = X bsl N + Acc,
    <<_:Length/binary, Rest2/binary>> = Rest,
    dfp_read_field_def_init_response(Rest2, 0, 0, F@_1,
				     TrUserData).

skip_group_init_response(Bin, FNum, Z2, F@_1,
			 TrUserData) ->
    {_, Rest} = read_group(Bin, FNum),
    dfp_read_field_def_init_response(Rest, 0, Z2, F@_1,
				     TrUserData).

skip_32_init_response(<<_:32, Rest/binary>>, Z1, Z2,
		      F@_1, TrUserData) ->
    dfp_read_field_def_init_response(Rest, Z1, Z2, F@_1,
				     TrUserData).

skip_64_init_response(<<_:64, Rest/binary>>, Z1, Z2,
		      F@_1, TrUserData) ->
    dfp_read_field_def_init_response(Rest, Z1, Z2, F@_1,
				     TrUserData).
...
merge_msgs(Prev, New, MsgName, Opts) ->
    ...
    case MsgName of
      ...
      init_response ->
	  merge_msg_init_response(Prev, New, TrUserData);
      client_connect_request ->
...
-compile({nowarn_unused_function,merge_msg_init_response/3}).
merge_msg_init_response(PMsg, NMsg, TrUserData) ->
    S1 = #{},
    case {PMsg, NMsg} of
...
verify_msg(Msg, MsgName, Opts) ->
    ...
    case MsgName of
      ...
      init_response ->
	  v_msg_init_response(Msg, [MsgName], TrUserData);
      client_connect_request ->
...
-compile({nowarn_unused_function,v_msg_init_response/3}).
-dialyzer({nowarn_function,v_msg_init_response/3}).
v_msg_init_response(#{} = M, Path, TrUserData) ->
    case M of
      #{hooks := F1} ->
	  if is_list(F1) ->
    ...
    lists:foreach(fun (hooks) -> ok;
		  ...
		  maps:keys(M)),
    ok;
v_msg_init_response(M, Path, _TrUserData)
    when is_map(M) ->
    mk_type_error({missing_fields, [] -- maps:keys(M),
		   init_response},
		  M, Path);
v_msg_init_response(X, Path, _TrUserData) ->
    mk_type_error({expected_msg, init_response}, X, Path).
...
get_msg_names() ->
    [initial_request, init_response, client_connect_request,
...
get_service_def('emqx.exhook.v1.ExHook') ->
	...
	output => init_response, input_stream => false,
...
fqbin_to_msg_name(<<"emqx.exhook.v1.InitResponse">>) -> init_response;
...
msg_name_to_fqbin(init_response) -> <<"emqx.exhook.v1.InitResponse">>;
...
get_msg_containment("exhook") ->
    [bool_result, client_authenticate_request,
     ...
     client_subscribe_request, client_unsubscribe_request,
     conn_info, empty_success, init_response,
     initial_request, message, message_acked_request,
...
get_proto_by_msg_name_as_fqbin(<<"emqx.exhook.v1.InitResponse">>) -> "exhook";
